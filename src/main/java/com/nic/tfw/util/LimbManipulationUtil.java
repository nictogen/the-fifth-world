...
public class LimbManipulationUtil
{
	...
	{
		return getLimbManipulator(model, limb, model);
	}

	public static LimbManipulator getLimbManipulator(ModelPlayer model, Limb limb, ModelPlayer copying)
	{
		LimbManipulator manipulator = new LimbManipulator();
		try
		{
			...
			ModelRenderer oldRenderer = (ModelRenderer) limb.rendererField.get(model);
			ModelRenderer modelRenderer = (ModelRenderer) limb.rendererField.get(copying);

			manipulator.limbs.add(new CustomModelRenderer(model, textureOffsetXField.getInt(modelRenderer), textureOffsetYField.getInt(modelRenderer), modelRenderer, oldRenderer, limb.rendererField));
			modelRenderer = (ModelRenderer) limb.secondaryRendererField.get(copying);
			manipulator.limbs.add(new CustomModelRenderer(model, textureOffsetXField.getInt(modelRenderer), textureOffsetYField.getInt(modelRenderer), modelRenderer, oldRenderer, limb.secondaryRendererField));
	...
	public static class CustomModelRenderer extends ModelRenderer
	{
		...
		{
			this(model, texOffX, texOffY, old, old, field);
		}

		private CustomModelRenderer(ModelBiped model, int texOffX, int texOffY, ModelRenderer copying, ModelRenderer old,
				Field field) throws IllegalAccessException
		{
			super(model, "");
			...
			this.f = field;
			this.cubeList = copying.cubeList;
			this.setRotationPoint(copying.rotationPointX, copying.rotationPointY, copying.rotationPointZ);
