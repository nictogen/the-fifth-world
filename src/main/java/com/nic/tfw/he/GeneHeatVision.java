 ...
 */
public class GeneHeatVision extends Gene
{
	private static final String COLOR_TAG = "color";

	public GeneHeatVision()
	{
		super(AbilityHeatVision.class, "Heat Vision");
	}
	...
	@Override public Ability getAbility(EntityLivingBase entity, GeneSet.GeneData geneData)
	{
		...
		a.getDataManager().set(
				AbilityHeatVision.COLOR, new Color(Math.min(255, Math.max(color[0], 0)), Math.min(255, Math.max(color[1], 0)), Math.min(255, Math.max(color[2], 0))));
		return a;
	}
